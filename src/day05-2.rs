...
fn solution() -> Result<String> {
    ...
    for mv in moves {
        let idx = stacks[mv.from - 1].len() - mv.count;
        let boxes2move = stacks[mv.from - 1].split_off(idx);
        stacks[mv.to - 1].extend(boxes2move.into_iter());
    }
...
mod tests {
    ...
    #[test]
    fn sln() {
        assert_eq!(solution().unwrap(), "GGNPJBTTR");
    }
}
